    ...
    try:
        ...
        if Config.LOGGER_ID != 0:
            await bot.send_file(
                Config.LOGGER_ID,
                LEGEND_PIC,
                caption=f"Deployed Lêɠêɳ̃dẞø† Successfully\n\nLêɠêɳ̃dẞø† ~ {LEGENDversion}\n\nType `{l1}help` or `{l1}ping` to check!\nFor Assistant Type `.on` \n\nJoin [LegendBot Channel](t.me/LegendBotUb) for Updates & [LegendBot Chat](t.me/Legend_Userbots) for any query regarding LegendBot",
            )
    except Exception as e:
        print(str(e))

    try:
        await bot(JoinChannelRequest("@LegendBotUb"))
    except BaseException:
        pass

    try:
        await bot(JoinChannelRequest("@Legend_Userbots"))
    except BaseException:
        pass
